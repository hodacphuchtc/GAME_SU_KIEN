/*
 * NGUỒN: chép từ `modules/GAME_SU_KIEN/app/lib/bao-ve/phien-quan-tri.ts` @ 3d96358.
 * ĐÃ SỬA cho Vòng Quay: bỏ hẳn nhân viên/cơ sở/vai trò — bản này chỉ có MỘT mật
 * khẩu quản trị (xem mục "KHÔNG LÀM Ở PHIÊN BẢN NÀY" trong PLAN_VONG_QUAY.md), và
 * đổi tên biến môi trường sang `VONG_QUAY_KHOA_PHIEN` để hai app không dùng chung
 * khoá — chung khoá thì một cookie ký ở app này mở được cửa app kia.
 */
...
export const TEN_COOKIE = "vqmm_phien";
 ...
 *
 * Ngưỡng 32 ký tự là sàn cứng: khoá ngắn hơn thì HMAC-SHA256 mất phần lớn ý
 * nghĩa, và một khoá kiểu "vongquay2026" dò ra trong vài phút.
 */
function khoaBiMat(): string | null {
  const k = process.env.VONG_QUAY_KHOA_PHIEN;
...
/** Đã cấu hình mật khẩu quản trị chưa. Cùng lý do như trên: báo, không lộ. */
export function daCoMatKhauQuanTri(): boolean {
  const v = process.env.VONG_QUAY_MAT_KHAU_BAM;
 ...
 *
 * 🔴 `secure` chỉ bật khi thật sự chạy HTTPS. Máy ở quầy chạy `http://192.168.x.x`
 * cho cả LCD lẫn điện thoại; bật `secure` ở đó thì trình duyệt **lặng lẽ vứt
 * cookie** — người vận hành gõ đúng mật khẩu, trang nháy một cái rồi quay về
 * đúng màn đăng nhập, mãi mãi, và không một dòng lỗi nào giải thích.
 */
export function thuocTinhCookie(): {
  ...
  return {
    ...
    secure: process.env.NODE_ENV === "production" && process.env.VONG_QUAY_HTTPS === "1",
  };
}
