...
import { danhSachVongQuay, type TrangThaiChuongTrinh } from "@/lib/chuong-trinh/kho";
...
export default async function TrangVongQuay() {
  ...
  const ds = danhSachVongQuay(phamViCua(nguoi));

  return (
    <div className="mx-auto max-w-5xl">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-black text-muc sm:text-3xl">{T.vongQuayTitle}</h1>
          <p className="mt-1 text-sm text-chi">{T.vongQuaySubtitle}</p>
        </div>
        <Link
          href="/quan-tri/vong-quay/tao"
              ...
              >
                <div>
                  ...
                  <p className="mt-0.5 text-sm text-chi">
                    {ct.tenTrungTam} · {T.vongQuayLuotDaQuay}: <b className="text-muc">{ct.soLuot}</b>
                  </p>
                </div>
