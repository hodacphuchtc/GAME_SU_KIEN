...
export default async function TrangDungLai({
  ...
  // 🔴 Lọc PHẠM VI ngay tại đây. Đường dẫn ghép tay không được phép mở sổ của
  // cơ sở khác — lớp chặn ở `proxy.ts` chỉ hỏi "đã đăng nhập chưa".
  const nguoi = await batBuocDangNhap();
  const ct = timTheoMaVongQuay(ma, phamViCua(nguoi));
  if (!ct) notFound();

  const l = timLuotQuay(Number(luot));
